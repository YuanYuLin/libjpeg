...
def MAIN_EXTRACT(args):
    ...
    ops.copyto(ops.path_join(src_usr_lib_dir, "libjpeg.so.62.2.0"), dst_lib_dir)
    ops.ln(dst_lib_dir, "libjpeg.so.62.2.0", "libjpeg.so.62.2")
    ops.ln(dst_lib_dir, "libjpeg.so.62.2.0", "libjpeg.so.62")
    ops.ln(dst_lib_dir, "libjpeg.so.62.2.0", "libjpeg.so")


    ops.mkdir(tmp_include_dir)
    ops.copyto(ops.path_join(src_include_dir, 'jerror.h'), tmp_include_dir)
    ops.copyto(ops.path_join(src_include_dir, 'jmorecfg.h'), tmp_include_dir)
    ops.copyto(ops.path_join(src_include_dir, 'jpegint.h'), tmp_include_dir)
    ops.copyto(ops.path_join(src_include_dir, 'jpeglib.h'), tmp_include_dir)
    ops.copyto(ops.path_join(src_include_dir, 'x86_64-linux-gnu/jconfig.h'), tmp_include_dir)
    return True
